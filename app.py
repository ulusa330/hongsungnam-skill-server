...
SCHEDULE_KEYWORDS = ['강의 일정', '특강 일정', '다음 강의', '강의 날짜', '다음 특강', '몇월', '몇 월', '다음 특강 언제', '특강 언제']
# 아래는 제외: "특강 있나요/특강 있어요/특강 있"는 "{주제} 특강 있나요?"처럼
# 주제어가 앞에 붙어도 그대로 매칭되어, 내용 질문을 일정 안내로 잘못
# 가로채는 문제가 있었음 (예: "내사 관련 특강 있나요?" -> 다음 특강 날짜만 답함)
...
LECTURE_TOPIC_STOPWORDS = [
    ...
    '관련', '내용', '얘기', '이야기', '주제', '에서',
    # 멀티턴에서 직전 발화와 이어붙일 때 섞여 들어오는 지시어
    # (홑글자 그/이/저는 다른 단어 속에 우연히 포함될 위험이 있어 제외)
    '해당', '그거', '이거', '저거', '그것', '이것', '저것', '그런', '이런', '저런',
    '있나요', '있어요', '있을까요', '있습니까', '있는지', '있나', '있는거', '있던',
    '봤나요', '봤어요', '보여줘', '보여주세요', '보여줄래',
    '알려줘', '알려주세요', '알려줄래', '알려줄',
    '찾아줘', '찾아주세요', '찾아줄래', '찾아봐줘', '찾아봐',
    '요약해줘', '요약해 줘', '요약해주세요', '요약 해줘',
    '올라온거', '올라온', '올라왔나요', '올라왔어요', '올려진',
    '나온거', '나온', '다룬', '다뤘던', '다뤘',
    '언급한', '언급된', '말씀하신', '말한',
    '해줘', '해주세요', '해줄래', '특강', '영상',
]
...
- 여행, 관광, 장소 안내, 교통, 숙박, 비행기 등 생활 정보는 답변하지 말 것
- 범위 밖 "질문"(예: 여행지 추천, 요리법 등 실제로 정보를 구하는 것)은
  반드시 "해당 내용은 홍성남 신부님 채널에서 제공하고 있지 않습니다."라고만 답할 것
- 단, 인사("안녕하세요", "안녕히 계세요" 등), 감사 표현("고마워", "감사합니다" 등),
  대화를 마무리/전환하려는 표현("다른 얘기 하고 싶어", "이제 그만할래", "수고하세요" 등)은
  실제 정보를 구하는 "질문"이 아니므로, 이런 경우에는 절대 위 거부 문구를 쓰지 말고
  짧고 자연스럽게 응답할 것 (예: "네, 편하게 다른 이야기 해주세요" "네, 언제든 다시 찾아주세요")
  — 판단 기준은 "정보를 요청하는 질문인가 아닌가"이지, 특정 예시 문구와 정확히 일치하는지가 아님
